...
@app.get("/api/models", response_model=AssetListResponse)
        ...
        with get_db_connection() as conn:
            ...
            if 'usdz' in files and resolution in files['usdz']:
                ...
                if isinstance(usdz_entry, dict) and 'usdz' in usdz_entry and 'url' in usdz_entry['usdz']:
                    ...
                    raw_name = str(row.get('external_id') or row['id']); safe_name = re.sub(r'[^A-Za-z0-9._-]', '_', raw_name); safe_name = safe_name.lstrip('.')
                    ...
                    if not usdz_path.exists():
                        ...
                        tmp_path = usdz_path.with_suffix(".tmp")
                        if not download_to_path(usdz_url, tmp_path):
                        ...
                        if not usdz_path.exists():
                            ...
                            try:
                                with tempfile.TemporaryDirectory() as tmpdir:
                                    ...
                                    # Download included files preserving paths (retry up to 2 times)
                                    failed_includes = []
                                    for relpath, info in includes.items():
                                        ...
                                        for attempt in range(3):
                                            try:
                                                logger.info(f"Downloading include {inc_url} -> {dest_path} (attempt {attempt+1})")
                                                ...
                                                logger.warning(f"Include download attempt {attempt+1} failed for {relpath}: {e}")
                                        if not downloaded:
                                            failed_includes.append(relpath)
                                    if failed_includes:
                                        raise HTTPException(status_code=500, detail=f"USDZ assembly incomplete — failed to download: {failed_includes}")

                                    # Create USDZ by zipping all files in tmpdir (write to .tmp then rename atomically)
                                    import zipfile
                                    usdz_tmp_path = usdz_path.with_suffix(".tmp")
                                    logger.info(f"Creating USDZ at {usdz_path}")
                                    # Use compressed archive for non-USD files, but store USD uncompressed
                                    with zipfile.ZipFile(usdz_tmp_path, 'w', compression=zipfile.ZIP_DEFLATED) as zf:
                                    ...
                                    os.replace(usdz_tmp_path, usdz_path)
            ...
            if found and found_url:
                ...
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    CONVERTER_ARGV: dict[str, list] = {
                        "gltf2usd": ["gltf2usd", "{input}", "{output}"],
                        "usdzconvert": ["usdzconvert", "{input}", "-o", "{output}"],
                        "obj2usdz": ["usdzconvert", "{input}", "-o", "{output}"],
                        "FBX2glTF": ["FBX2glTF", "--binary", "--input", "{input}", "--output", "{output}"],
                        "blender": [
                            "blender", "--background", "--python-expr",
                            "import bpy,sys; bpy.ops.wm.open_mainfile(filepath=sys.argv[-2]); bpy.ops.wm.usd_export(filepath=sys.argv[-1])",
                            "--", "{input}", "{output}",
                        ],
                    }

                    tool_name = os.environ.get('CONVERSION_TOOL', '').strip()
                    argv_template = CONVERTER_ARGV.get(tool_name)
                    if not argv_template:
                        raise HTTPException(
                            status_code=501,
                            detail=(
                                f"No supported conversion tool configured (CONVERSION_TOOL={tool_name!r}). "
                                f"Set CONVERSION_TOOL to one of: {', '.join(CONVERTER_ARGV)}."
                            ),
                        )

                    out_usd = tmpdir_path / f"{safe_name}.usd"
                    argv = [
                        a.replace("{input}", str(src_path)).replace("{output}", str(out_usd))
                        for a in argv_template
                    ]
                    try:
                        logger.info(f"Running converter {tool_name}: {argv}")
                        completed = subprocess.run(argv, cwd=str(tmpdir_path), stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=300)
                        if completed.returncode != 0:
                            logger.error(f"Conversion failed: {completed.stderr.decode()}")
...
FLASHCARDS_BACKEND_URL = os.environ.get("FLASHCARDS_BACKEND_URL", "http://localhost:8000").rstrip("/")
...
async def get_flashcards(
    ...
    """Proxy flashcard review cards from main backend."""
    try:
        url = f"{FLASHCARDS_BACKEND_URL}/api/flashcards/review"
        headers = {}
        if authorization:
        ...
        if resp.status_code != 200:
            logger.error(f"Flashcards backend error: {resp.status_code} {resp.text[:200]}")
...
if __name__ == "__main__":
    ...
    print("Connected to PostgreSQL database with 2000+ models")
