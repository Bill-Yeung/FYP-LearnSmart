...
extension Entity {
    ...
    static func createSkybox(from url: URL, radius: Float = 1000) async throws -> Entity {
        let texture: TextureResource
        if url.isFileURL {
            texture = try await TextureResource(contentsOf: url)
        } else {
            let (tempURL, _) = try await URLSession.shared.download(from: url)
            let fileExtension = url.pathExtension.isEmpty ? "hdr" : url.pathExtension
            ...
            texture = try await TextureResource(contentsOf: preservedURL)
        }

        let mesh = MeshResource.generateSphere(radius: radius)
        var material = UnlitMaterial()
        material.color = .init(texture: .init(texture))
        // Render back-faces (inside of sphere) so the user inside sees the texture.
        // scale.x=-1 is unreliable because RealityKit may not flip baked normals under
        // negative scale, leaving back-faces culled and the sphere invisible.
