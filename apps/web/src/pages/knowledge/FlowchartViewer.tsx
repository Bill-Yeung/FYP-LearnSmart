...
// topo-sort the nodes into layers and lay each layer out left-to-right
...
// draw a smooth s-curve svg path from one node bottom to the next node top
...
export const FlowchartViewer = forwardRef(function FlowchartViewer({ nodes: propNodes, links: propLinks, viewState, onSave, title: _title, onUnsavedChange }: FlowchartViewerProps, ref: React.ForwardedRef<DiagramViewerHandle>) {
  ...
  // attach a non-passive wheel listener so we can preventDefault for zoom
  useEffect(() => {
    const svgEl = svgRef.current
    if (!svgEl) return
  ...
  // when in link-creation mode, pick the second node and open the relationship-type modal
