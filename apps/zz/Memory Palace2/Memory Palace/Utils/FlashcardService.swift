...
final class FlashcardService {
    ...
    private func fetchFromBackend(accessToken: String) async throws -> [ReviewCard] {
        ...
        do {
            ...
            let decoder = JSONDecoder()
            // Backend wraps results: {"cards": [...], "total": N, ...}
            if let wrapper = try? decoder.decode([String: [ReviewCard]].self, from: data) {
                for key in ["cards", "flashcards", "data", "results"] {
                    if let arr = wrapper[key] { return arr }
                }
            }
            print("Decoding error: \(error)")
            if let body = String(data: data, encoding: .utf8) { print("Response body: \(body)") }
