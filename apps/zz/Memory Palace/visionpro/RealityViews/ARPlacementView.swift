...
struct ARPlacementView: View {
        ...
        if item.displayType == "3d_model", let assetId = item.assetId {
            ...
            Task {
                ...
                do {
                    ...
                    // Apply real PBR materials (Polyhaven MaterialX → RealityKit PBR)
                    Self.applyMaterialsRecursive(to: model, diffuseURL: asset.diffuseURL, roughnessURL: asset.roughnessURL)
    ...
    private static func applyMaterialsRecursive(to entity: Entity, diffuseURL: URL?, roughnessURL: URL?) {
        // Load textures synchronously from local cache files (already downloaded)
        ...
        applyMaterials(to: entity, diffuse: diffuseTex, roughness: roughnessTex)
    }

    private static func applyMaterials(to entity: Entity, diffuse: TextureResource?, roughness: TextureResource?) {
        ...
        for child in entity.children {
            applyMaterials(to: child, diffuse: diffuse, roughness: roughness)
        }
    }
}
