...
class PalaceViewModel {
    ...
    func createPalace(name: String, description: String?, mode: String, skyboxType: String, skyboxPreset: String?, skyboxImageUrl: String? = nil) async {
        isLoading = true
        errorMessage = nil
        do {
    ...
    func removeItem(itemId: String) async {
        guard let palace = currentPalace else {
            print("[PalaceVM] removeItem failed — currentPalace is nil")
            return
        }
        print("[PalaceVM] removing item \(itemId) from palace \(palace.id)")
        do {
            try await api.deleteItem(palaceId: palace.id, itemId: itemId)
            items.removeAll { $0.id == itemId }
            print("[PalaceVM] item \(itemId) removed successfully")
        } catch {
            print("[PalaceVM] removeItem error: \(error)")
            errorMessage = error.localizedDescription
        }
    }
    ...
    func submitReview(itemId: String, quality: Int) async {
        ...
        do {
            ...
            // Refresh items so the updated nextReviewAt is reflected in the UI
            let refreshed = try await api.getPalaceItems(palaceId: palace.id)
            print("[PalaceVM] after review, item nextReviewAt:", refreshed.first(where: { $0.id == itemId })?.nextReviewAt ?? "nil")
            items = refreshed
        } catch {
            print("[PalaceVM] submitReview error:", error)
            errorMessage = error.localizedDescription
        }
    }
}
