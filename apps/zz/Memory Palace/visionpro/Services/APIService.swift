...
class APIService {
    ...
    func createPalace(name: String, description: String?, mode: String, skyboxType: String, skyboxPreset: String?, skyboxImageUrl: String? = nil) async throws -> MemoryPalace {
        ...
        if let u = skyboxImageUrl { body["skybox_image_url"] = u }
    ...
    struct VisionProPreset: Codable, Identifiable {
        var id: String { name }
        let name: String
        let displayName: String
    ...
    func listScenePresets() async throws -> [VisionProPreset] {
        let data = try await get("/api/visionpro/scene/presets")
        return try decode([VisionProPreset].self, from: data)
    }
    ...
    func deleteFlashcard(id: String) async throws {
        _ = try await delete("/api/flashcards/\(id)")
    }
