...
BEGIN
  IF TG_OP = 'INSERT' THEN
    UPDATE shared_content SET like_count = like_count + 1 WHERE id = NEW.shared_content_id;
  ELSIF TG_OP = 'DELETE' THEN
    UPDATE shared_content SET like_count = like_count - 1 WHERE id = OLD.shared_content_id;
  END IF;
  RETURN NULL;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trigger_content_like_count
